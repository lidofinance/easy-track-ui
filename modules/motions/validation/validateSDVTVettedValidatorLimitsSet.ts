...
export const validateSDVTVettedValidatorLimitsSet: MotionValidator =
  async args => {
    // Check MF0604: Same node operator in concurrent motions
    const activeMotionsValidation =
      await validateConcurrentMotions<SetVettedValidatorsLimitsAbi>({
        motionType: MotionTypeForms.SDVTVettedValidatorsLimitsSet,
